...
#selectedWords
barplot(wordCounts[wordFrequency >= 50], las = 2, cex.names = 0.5)
```
...
percentagePos <- (sum(matchedP)/length(wordCounts))*100
percentagePos
#length(wordCounts)
#sum(matchedP)
...
percentageNeg <- (sum(matchedN)/length(wordCounts))*100
percentageNeg
