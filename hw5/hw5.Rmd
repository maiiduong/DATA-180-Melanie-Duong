...
```{r}
final_vans_Euclidean <- round(Vans_Euclidean, 2)
final_vans_Euclidean

min_dist <- min(dist(final_vans_Euclidean))
min_dist

max_dist <- max(dist(final_vans_Euclidean))
max_dist
...
```{r}
min_dist <- min(dist(final_vans_Euclidean, method = "manhattan"))
min_dist

max_dist <- max(dist(final_vans_Euclidean, method = "manhattan"))
max_dist
```
...
d43 = 5
d45 = 2
dSAB = min(d12, d13, d15, d42, d43, d45)
dSAB
```
...
d43 = 5
d45 = 2
dSAB = max(d12, d13, d15, d42, d43, d45)
dSAB
```
...
d43 = 5
d45 = 2
dSAB = (d12 + d13 + d15 + d42 + d43 + d45)/(2*3)
dSAB
```
...
I cut the dendogram at the middle to get 5 clusters and the members of each are listed above. 
...
```{r}
vertebrate <- read.csv("C:/Users/maidn/OneDrive/Desktop/DATA 180 Melanie Duong/DATA-180-Melanie-Duong/DATA-180-Melanie-Duong/hw5/vertebrate.csv")
nrow(vertebrate)
vertebrate
```
...
```{r}
vertebrate_Eu_complete <- hclust(dist(vertebrate))
vertebrate_Eu_complete
plot(vertebrate_Eu_complete, cex = .75, xlab = "")

vertebrate_Eu_single <- hclust(dist(vertebrate), method = "single")
vertebrate_Eu_single
plot(vertebrate_Eu_single, cex = .75, xlab = "")
```
...
```{r}
ver_man_single <- hclust(dist(vertebrate, method = "manhattan"), method = "single")
ver_man_single
plot(ver_man_single, lwd = 3, main = "Clustered vertebrates, Manhattan, Single linkage", sub = "", xlab = "Vertebrates", cex.lab = 1.25, hang = -1)
```

Which method do you prefer?
I would prefer to use Euclidean and Single linkage method. 
...
```{r}
library(ggdendro)
ver_dendo <- as.matrix(dist(vertebrate[,-1]))
dd <- dist(scale(ver_dendo), method = "manhattan")
