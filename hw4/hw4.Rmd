...
```{r}
mean_values <- records %>%group_by(track) %>% summarise(mean_time = mean(time))
mean_values
min_value <- min(mean_values$mean_time)
fastest_track <- select(filter(mean_values, mean_time == min_value), track)
