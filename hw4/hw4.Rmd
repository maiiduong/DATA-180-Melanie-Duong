...
```{r}
mean_values <- records %>%group_by(year) %>% summarise(mean_recordtime = mean(time))
mean_values
```
# Question 7
...
```{r, fig.width=10}
library(ggplot2)
average_record <- records %>%group_by(year) %>% summarise(mean_recordtime = mean(time)) %>%ggplot(aes(year, mean_recordtime, fill = year)) + geom_bar(stat = 'identity') + labs(title =  "Average Record Time by Year", x = "Year", y = "Average time record")
average_record
