...
```{r}
record_cases <- select(filter(records, record_duration > 365))
count_cases <- nrow(record_cases)
count_cases
```
# Question 3
...
```{r}
max_record <- max(records$record_duration)
max_record
longest_record <- select(filter(records, record_duration == max_record), track, player, date, record_duration)
longest_record 
```
...
```{r}
mean_values <- records %>%group_by(track) %>% summarise(mean_recordtime = mean(time))
mean_values
min_value <- min(mean_values$mean_recordtime)
fastest_track <- select(filter(mean_values, mean_recordtime == min_value), track)
...
```{r}
year_record <- mutate(records, year = substr(date, 1, 4))
year_record
```
...
```{r}
mean_values <- year_record %>%group_by(year) %>% summarise(mean_recordtime = mean(time))
mean_values
```
# Question 7
...
```{r, fig.width=10}
library(ggplot2)
average_record <- year_record %>%group_by(year) %>% summarise(mean_recordtime = mean(time)) %>%ggplot(aes(year, mean_recordtime, fill = year)) + geom_bar(stat = 'identity') + labs(title =  "Average Record Time by Year", x = "Year", y = "Average time record")
average_record

# Overall, there is not a specific trend during the period of time. The record times fluctuated, hit the bottom in 2007 and reached the peak 10 years later. 
...
```{r}
library(ggplot2)
average_recordtime <- records %>%ggplot(aes(x = time, fill = type)) + geom_histogram(alpha = .5, bins = 30) + labs(title = "Distribution of Record Time by Track Type", x = "Record Time", y = "Frequency")
average_recordtime
```
...
```{r}
library(ggplot2)
time_perlap <- mutate(records, time_per = ifelse(type == "Three Lap", time / 3, time))
perlap_performance <- time_perlap %>%ggplot(aes(x = time_per, fill = type)) + geom_histogram(position = "identity", alpha = .5, bins = 30) + labs(title = "Distribution of Record Time per Type", x = "Record Time per Lap", y = "Frequency")
perlap_performance
...
library(ggplot2)
time_perlap <- mutate(records, time_per = ifelse(type == "Three Lap", time / 3, time))
perlap_performance <- time_perlap %>% filter(shortcut == "No") %>% ggplot(aes(x = time_per, fill = type)) + geom_histogram(position = "identity", alpha = .5, bins = 30) + labs(title = "Distribution of Record Time per Type", x = "Record Time per Lap", y = "Frequency")
perlap_performance
```
...
```{r}
mean_time <- records %>%group_by(system_played) %>%summarise(mean_time = mean(time))
mean_time
#There are differences between the two system played. The mean time record of NTSC is around 79.9 while that of PAL is approximately 95.2. Therefore, it might affect performance of records set.
```
